...
namespace NBADailyFantasyOptimizer.DataAccess
{
    public class PlayerDao
    {
        public List<PlayerDto> GetAllPlayers(int day)
        {
            ...
            var projections = GetAllPlayerProjections(day, players).ToList();
            var previousProjections = new List<ProjectionDto>();
            Enumerable.Range(day - 15, 16).ToList().ForEach(s => previousProjections.AddRange(GetAllPlayerProjections(s, new List<PlayerDto>())));

            List<ActualResultsDto> actuals = new List<ActualResultsDto>();
            Enumerable.Range(day - 6, 7).ToList().ForEach(s => actuals.AddRange(GetAllPlayerActuals(s)));
        ...
        {
            var filePath = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBASalariesDay{0}.csv", day); 
        ...
        public IEnumerable<ProjectionDto> GetAllPlayerProjections(int day, List<PlayerDto> players)
        {
            var filePath = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAProjectionsDay{0}.csv", day);
            if (!System.IO.File.Exists(filePath))
            ...
            var filePath2 = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAProjectionsDay{0}-NumberFire.csv", day);
            ...
            var filePath3 = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAProjectionsDay{0}-SaberSim.csv", day);
            ...
            var filePath4 = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAProjectionsDay{0}-FC.csv", day);
            ...
            var filePath5 = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAProjectionsDay{0}-rg.csv", day);
            if (System.IO.File.Exists(filePath5))
            {
                var lines2 = System.IO.File.ReadAllLines(filePath5).ToList();
                ...
                //foreach (var projj in RGProjs.Where(s => s.Dvp > 10))
                //{
                //    var avgSamePosDvp = RGProjs.Where(s => s.Position == projj.Position && s.Dvp > 10).Average(s => s.Dvp);
                //    projj.Projection = projj.Projection * (Math.Pow((projj.Dvp / avgSamePosDvp), 2));
                //}
            }
            ...
            foreach (var proj in dfnProjs)
            {
                ...
                var matchingPlayer = players.Find(p => p.Name == proj.Name);
                ...
                all.Add(proj);
                //if (saberMatch != null && matchingPlayer != null && (matchingPlayer.Salary < 15 || matchingPlayer.Salary > 45))
                //    all.Add(saberMatch);
              
                //if (fcMatch != null && matchingPlayer != null && (matchingPlayer.Salary < 15 || matchingPlayer.Salary > 45))
                //    all.Add(fcMatch);
                if (rgMatch != null)
                {
                    all.Add(rgMatch);
                ...
                //if (all.Count() > 2)
                //{
                //    all.Remove(all.OrderByDescending(o => Math.Abs(o.Projection - all.Average(f => f.Projection))).First());
                //}


                //var fileToUse = ConstantDto.GetProjToUseBasedOnMostAccurateForGroup(saberMatch.Position, all.Average(s => s.Projection));

                //if (fileToUse == ConstantDto.DFNFileName)
                //    continue;
                //if (fileToUse == ConstantDto.NFFIleName && nfMatch != null)
                //    proj.Projection = nfMatch.Projection;
                //if (fileToUse == ConstantDto.SaberFileName && saberMatch != null)
                //    proj.Projection = saberMatch.Projection;
                //proj.Projection = nfMatch != null && ( fcMatch != null && fcMatch.Position == "C" || saberMatch != null && saberMatch.Position == "C") ? nfMatch.Projection : proj.Projection;
                proj.Projection = all.Any() ? all.Average(x => x.Projection) : proj.Projection;
                //if (bads.Count > 1)
                //    proj.Projection = (proj.Projection * all.Count + bads.Average(s => s.Projection)) / (all.Count + 1);
            }
            //foreach (var proj in dfnProjs)
            //{
            //    List<ProjectionDto> all = new List<ProjectionDto>();
            ...
            //    proj.Projection = all.Max(s => s.Projection);
            //}
            ...
            //foreach (var projj in dfnProjs.Where(s => s.Position != null))
            //{
            //    var avgSamePosDvp = dfnProjs.Where(s => s.Position == projj.Position && s.Dvp > 10).Average(s => s.Dvp);
            //    projj.Projection = projj.Projection * ((projj.Dvp) / (avgSamePosDvp ));
            //}
            //foreach (var proj in numberFireProjs)
            //{
            //    if (dfnProjs.All(p => p.Name != proj.Name))
            //        dfnProjs.Add(proj);
            //}
            //foreach (var proj in saberProjs)
            //{
            //    if (dfnProjs.All(p => p.Name != proj.Name))
            //        dfnProjs.Add(proj);
            //}
        ...
        {
            var filePath = string.Format(@"C:\Users\Jake\Desktop\NBADailyFantasyOptimizer\Inputs\NBAActualsDay{0}.csv", day);
            if (!System.IO.File.Exists(filePath))
